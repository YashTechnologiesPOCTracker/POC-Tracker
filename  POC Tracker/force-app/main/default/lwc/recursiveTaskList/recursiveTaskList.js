...
export default class RecursiveTaskList extends NavigationMixin(LightningElement) {
    @api parentId;
    @api subcompId;
    @api program;
    recordId;
    ...
    connectedCallback() {
        ...
        registerListener("refreshApexBackClicked", this.handleCall, this);
        registerListener("refreshList", this.handleCallRefresh, this);
        ...
        fireEvent(this.pageRef, 'subTaskRecursiveReportChart', reportData);
    ...
    handleCallRefresh(detail) {
        console.log("in handleCallback refresh" + detail);
        return refreshApex(this.refreshTable);
    }
        ...
        if (result.data) {
            ...
            newData.forEach(element => {
                ...
                if (element.State__c === 'Completed') {
                    ...
                    let newObject = {};
                    //this.subcompId = element.Subsidiary_CompetencyId__c
                    newObject.Id = element.Id;
                    newObject.Name = element.Name;
                    newObject.Title = element.Title__c;
                    newObject.Program = element.Program__c;
        ...
        if (this.isDrop) {
            ...
            Promise.all(
                recordInputs.map(recordInput => {
                    const fields = {};
                    console.log("SUBCOMPID::::: " + this.subcompId)
                ...
                fireEvent(this.pageRef, "editRecursiveReportUpdate", 'update sub report');
                //this.updateProgress();
                this.draftValues = [];
                this.isDrop = false;
                return refreshApex(this.refreshTable);
    ...
    handleRowAction(event) {
        ...
        switch (actionName) {
            case "view":
                console.log("record Details from recursive task " + JSON.stringify(recordDetails));
                const customEvent = new CustomEvent('recursivetasklist', { detail: recordDetails });
    ...
    deleteTask(currentRow) {
        ...
        delSelectedTask({ recordId: currentRow.Id })
            .then((result) => {
                ...
                fireEvent(this.pageRef, "deleteRecursiveReportUpdate", 'update sub report');
